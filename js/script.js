...
const searchInput = document.querySelector('nav input');
    ...
    let colorL = rs.getPropertyValue('--light-bg');
    let colorHoverL = rs.getPropertyValue('--light-bg-hover');
    let colorActiveL = rs.getPropertyValue('--light-bg-active');

    let colorD = rs.getPropertyValue('--dark-bg');
    let colorHoverD = rs.getPropertyValue('--dark-bg-hover');
    let colorActiveD = rs.getPropertyValue('--dark-bg-active');
        ...
        listadoFerias.insertAdjacentHTML('beforeend', `
            <div class="municipio-container">
                <div onclick="desplegar(this, true)" class="municipio inactive municipio-head color-mode-transition">
                    ...
                    <svg width="1.5rem" viewBox="0 0 24 24" class="arrowSvg color-mode-transition" fill="${isDark ? colorD:colorL}" xmlns="http://www.w3.org/2000/svg"> <path d="M5.70711 9.71069C5.31658 10.1012 5.31658 10.7344 5.70711 11.1249L10.5993 16.0123C11.3805 16.7927 12.6463 16.7924 13.4271 16.0117L18.3174 11.1213C18.708 10.7308 18.708 10.0976 18.3174 9.70708C17.9269 9.31655 17.2937 9.31655 16.9032 9.70708L12.7176 13.8927C12.3271 14.2833 11.6939 14.2832 11.3034 13.8927L7.12132 9.71069C6.7308 9.32016 6.09763 9.32016 5.70711 9.71069Z"></path></svg>    
                </div>
...
function updateColors() {
    ...
    let arrowSvg = document.querySelectorAll('.arrowSvg');
    ...
    if (currentColor === colorL) {
        menu.style.setProperty('color', colorL);
        arrowSvg.forEach(arrow => {
            arrow.setAttribute('fill', colorL);
        });
        nav.style.setProperty('box-shadow', '0px -2px 5px ' + colorL);
        userLocate.children[0].setAttribute('stroke', colorL);
        r.style.setProperty('--bg', colorD);
        r.style.setProperty('--bg-hover', colorHoverD);
        r.style.setProperty('--bg-active', colorActiveD);
    }else{
        menu.style.setProperty('color', colorD);
        arrowSvg.forEach(arrow => {
            arrow.setAttribute('fill', colorD);
        });
        nav.style.setProperty('box-shadow', '0px 0px 5px ' + colorD);
        userLocate.children[0].setAttribute('stroke', colorD);
        r.style.setProperty('--bg', colorL);
        r.style.setProperty('--bg-hover', colorHoverL);
        r.style.setProperty('--bg-active', colorActiveL);
    }
}
updateColors()